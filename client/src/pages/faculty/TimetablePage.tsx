...
import { useGetFacultyTimetableQuery } from '@/services/timetableApi';
import { useAppSelector } from '@/hooks/useAppDispatch';
import { selectCurrentUser } from '@/features/authSlice';
import { usePdfDownload } from '@/hooks/usePdfDownload';

export default function FacultyTimetablePage(): React.ReactElement {
  const user = useAppSelector(selectCurrentUser);
  const currentYear = new Date().getFullYear();
  const academicYear = `${currentYear - 1}-${String(currentYear).slice(2)}`;

  const { data, isLoading } = useGetFacultyTimetableQuery(
    { facultyId: user?._id ?? '', academicYear },
    { skip: !user }
  );
  ...
  const handlePdfDownload = (timetableId: string, semester: number) => {
    ...
    void download(url, `my-timetable-sem${semester}-${academicYear}.pdf`);
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="My Timetable"
        subtitle={`${user?.name ?? ''} — ${academicYear}`}
      />
      ...
      {!isLoading && (!data || data.length === 0) && (
        <div className="text-center py-16 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700">
          <p className="text-gray-400 dark:text-slate-500 text-sm">No published timetable found for {academicYear}.</p>
          <p className="text-gray-300 dark:text-slate-600 text-xs mt-1">Contact your HOD to generate and publish the timetable.</p>
        </div>
      )}
